import numpy as np
import yaml
from mink_pan.utils.eval_np import PanopticEval
...
class PanopticKittiEvaluator:
    def __init__(self, cfg=None):
        ...
        #dataset_config_file = "/home/penguin2/Documents/Strawberries/instance_segmentation/mink_panoptic/mink_pan/datasets/strawberries.yaml"
    ...
    def load_kitti_config(self, config_file):
        ...
        self.class_inv_lut = np.zeros((2), dtype=np.int32)
        ...
        self.stuff = ["unlabeled",]
        self.all_classes = self.things + self.stuff

    def update2(self, sem_preds, ins_preds, sem_label, ins_label):
        for i in range(len(sem_preds)):
            self.evaluator.addBatch(
                sem_preds,
                ins_preds,
                sem_label, #inputs["sem_label"][i].reshape(-1),
                ins_label, #inputs["ins_label"][i].reshape(-1),
                #inputs["fname"][i],
            )
        self.update_metrics()
    ...
    def print_results(self):
        ...
        #print("tps", self.evaluator.tps)
        #print("fps", self.evaluator.fps)
        #print("fns", self.evaluator.fns)

        tps = np.array(self.evaluator.tps)
        fps = np.array(self.evaluator.fps)
        fns = np.array(self.evaluator.fns)

        aps = np.where(fps+tps==0, 1, tps / (fps+tps))
