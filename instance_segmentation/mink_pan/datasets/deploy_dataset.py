...
class ExtremeInfo:
    def __init__(self, pcd, extension):
        print("generating extrema ", end="")
        self.extension = extension
        points = np.array(pcd.points)
        colors = np.array(pcd.colors)

        self.min_x = points[:, 0].min()
        self.max_x = points[:, 0].max()

        self.minpivot = self.min_x + self.extension[0]/2
        self.maxpivot = self.max_x - self.extension[0]/2
...
class SemanticDataset(Dataset):
    ...
    def __init__(self, data_path, cfg_path, split="train", seq=None, dataset="KITTI"):
        ...
        if self.test:
            ...
            self.labelpaths = [
                    f"{data_path}/reduced_14_21_2.npy"
                    ]
        else:
            self.paths = [
